        ...
        <div class="max-w-5xl mx-auto bg-white rounded-xl">
            <div class="flex flex-col lg:flex-row">
                <div class="lg:w-2/3 bg-gray-50 max-h-screen p-6">
                    ...
                    <div v-else-if="cart.data.length > 0" class="divide-y divide-gray-200">
                        <div v-for="(item, index) in cart.data" :key="index" class="flex items-center space-x-4 py-4">
...
export default {
    created() {
        this.$store.dispatch("fetchCarts", this.$auth.user.id);
    },
    computed: {
        cart() {
            return this.$store.getters.getCartState;
        },
        totalPrice() {
